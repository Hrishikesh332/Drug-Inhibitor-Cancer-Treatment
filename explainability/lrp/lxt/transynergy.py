...
class Norm(nn.Module):
    ...
    def forward(self, x):
        ...
        std = stop_gradient((var + self.eps).sqrt())
        
        y = (x - mean) / std
        y = y * self.alpha + self.bias
        return y


def attention(q, k, v, d_k = 1, mask=None):
...
class FeedForward(nn.Module):
    def __init__(self, d_model, d_ff=200, dropout=0.0):
        super().__init__()
...
class OutputFeedForward(nn.Module):
    ...
    def __init__(self, H, W, d_layers = None, dropout=0.0):
        ...
        # self.dropouts[0] = nn.Dropout(p=dropout) ### <------------------------------------------- LXT
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask=None, low_dim = False):
        ...
        x = x + self.ff(x2, low_dim = low_dim)
        return x
