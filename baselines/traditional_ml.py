...
def init_wandb(
    model_name: str, hyperam_suffix: str, fold_idx: int = None, paper: str = None
):
    wandb.init(
    ...
    table_val = wandb.Table(
        columns=[
            "Model Name",
            "Best Mean Val Score",
            "Sample Val Pearson Correlation",
            "Sample Val Spearman Correlation",
            "Best Val Params",
        ]
    )
...
def train_and_eval_model(
    ...
    table, test_table = init_wandb(
        model_name, "hyperparam_tuning", fold_idx=fold_idx, paper=paper
    )
    ...
    for params in tqdm(sampled_params):
        ...
        for X_train, y_train, X_val, y_val in tqdm(
            folds, desc="Inner folds", total=len(folds)
        ):
            try:
                model = train_model_with_timeout(
                    model_class, params, X_train, y_train, timeout=timeout
                )
            except TimeoutError:
        ...
        table.add_data(
            model_name, mean_val_score, pearson_corr_val, spearman_corr_val, str(params)
        )

        if (
            (best_val_score is None)
            or (mean_val_score < best_val_score)
            ...
            try:
                best_model = train_model_with_timeout(
                    model_class, params, X_all, y_all, timeout=timeout * 20
                )
                best_params = params
            except Exception as e:
                pass
            ...
            if output_path:
                joblib.dump(
                    model,
                    f"{output_path}/{model_name}_{hash(frozenset(params.items()))}.pkl",
                )
