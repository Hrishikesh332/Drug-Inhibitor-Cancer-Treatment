...
def setup_model_and_optimizer_with_params(reorder_tensor, params):
    ...
    drug_model = get_model_with_params(
        reorder_tensor.get_reordered_slice_indices(),
        input_masks=None,
        drugs_on_the_side=False,
        params=params,
    )
    optimizer = torch.optim.Adam(
        drug_model.parameters(),
        lr=setting.start_lr,
        weight_decay=setting.lr_decay,
        betas=(0.9, 0.98),
        eps=1e-9,
    )
...
def train_model_and_eval(
    ...
    if use_wandb:
        init_wandb(
            eval_idx, crossval=True, testing=testing, fold_col_name=fold_col_name
        )
