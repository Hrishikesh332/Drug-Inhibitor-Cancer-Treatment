...
class CustomizedLinearFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[1]:
            grad_weight = grad_output.permute(0,2,1).matmul(input).sum(0).squeeze(0)/(grad_output.size(0))
            if mask is not None:
                # change grad_weight to 0 where mask == 0
                grad_weight = grad_weight * mask
        #if bias is not None and ctx.needs_input_grad[2]:
        if ctx.needs_input_grad[2]:
            grad_bias = grad_output.sum(0).sum(0).squeeze(0)/(grad_output.size(0))
