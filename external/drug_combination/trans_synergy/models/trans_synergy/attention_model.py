...
from trans_synergy.models.trans_synergy.customized_linear import CustomizedLinear
from trans_synergy.models.trans_synergy.layers import DecoderLayer, EncoderLayer
from trans_synergy.models.trans_synergy.neural_finger_print.neural_fingerprint import (
    NeuralFingerprint,
)
from trans_synergy.models.trans_synergy.sublayers import Norm, OutputFeedForward
...
class FlexibleTransformer(Transformer):
    ...
    def forward(self, src_list, trg_list, src_mask=None, trg_mask=None, low_dim=False):
        ...
        assert len(self.linear_layers) == len(
            src_list
        ), "Features and sources length are different"
        final_srcs = []
        final_trgs = []
        ...
        for i in range(len(d_input_list)):
            ...
            for j in range(num_of_linear_module):
                (
                    ...
                    else self.linear_layers.append(
        ...
        for i in range(len(self.transformer_list)):
            ...
            for j in range(src_list[i].size(1)):
                ...
                cur_src_processed_dim = self.dropouts[cur_linear](
                    F.relu(self.linear_layers[cur_linear](cur_src_dim))
                )
                cur_trg_processed_dim = self.dropouts[cur_linear](
                    F.relu(self.linear_layers[cur_linear](cur_trg_dim))
                )
                src_list_dim.append(
...
class ChemFP(nn.Module):
    ...
    def __init__(self, device):
        ...
        if self.feature_map is None:
            self.feature_map = pd.read_csv(
                setting.chemfp_drug_feature_file, index_col=0
            )
        linear_layers = []
    ...
    def forward(self, drug_names):
        input_feature = (
            torch.from_numpy(self.feature_map.loc[list(drug_names)].values)
            .float()
            .to(self.device)
        )
    ...
    def __init__(
        ...
        if setting.neural_fp:
            self.drug_fp_a = NeuralFingerprint(
                ...
                setting.conv_size,
                setting.drug_emb_dim,
                setting.degree,
                device=self.device1,
            )
            self.drug_fp_b = NeuralFingerprint(
                setting.drug_input_dim["atom"],
                setting.drug_input_dim["bond"],
                setting.conv_size,
                setting.drug_emb_dim,
